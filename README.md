...
- Orders Management (CRUD theo product/quantity/discount)
- Profile (chỉnh sửa thông tin cá nhân + tổng quan đơn)
...
2. Nếu project Supabase đã từng được tạo với schema cũ, chạy `supabase/reset_schema.sql` trước để xoá các bảng lệch và dựng lại đúng cấu trúc theo ERD.
3. Nếu là project mới, chỉ cần chạy `supabase/schema.sql`.
...
Role admin/user được lưu trong Supabase Auth metadata, còn thông tin cá nhân nằm trong `people`.
...
- Tài khoản có trong `auth.users` nhưng chưa có dòng trong `people`.
...
5. Sai schema/table:
- Nếu project đã có schema cũ, chạy `supabase/reset_schema.sql` trước rồi mới chạy `supabase/seed.sql`.
- Sau khi apply SQL, nếu Supabase vẫn báo thiếu bảng, refresh/reload schema cache của project rồi thử lại.
